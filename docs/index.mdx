...
<CardGroup cols={2}>
  <Card title="Need a transport and config" icon="plug" href="/concepts/transports-and-runtime">
    Learn how HTTP, stdio, `/healthz`, and the attach sidecar fit together.
  </Card>
  <Card title="Need session semantics" icon="layers-3" href="/concepts/sessions">
    Understand `session_id`, default session behavior, runtime directories, and isolation.
  </Card>
  <Card title="Need path rules" icon="folder" href="/concepts/workspace-paths">
    See how host paths map into `/workspace` and when relative paths are rewritten.
  </Card>
  <Card title="Need tool outputs" icon="file-code" href="/concepts/response-shapes">
    Review dict responses, JSON-string responses, and error envelopes before integrating with an agent.
  </Card>
</CardGroup>
