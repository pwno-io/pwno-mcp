...
class SubprocessTools:
    ...
    def run_command(
        self, command: str, cwd: str | None = None, env: dict[str, str] | None = None, timeout: float | None = 30.0
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            # Execute command
            result = subprocess.run(cmd_parts, cwd=cwd, env=cmd_env, capture_output=True, text=True, timeout=timeout)
            ...
            return {"success": False, "command": command, "error": str(e), "cwd": cwd or os.getcwd()}

    def spawn_process(self, command: str, cwd: str | None = None, env: dict[str, str] | None = None) -> dict[str, Any]:
        """
        ...
        try:
            ...
            process = subprocess.Popen(
                cmd_parts, cwd=cwd, env=cmd_env, stdout=stdout_file, stderr=stderr_file, text=True
            )
        ...
        try:
            ...
            if pid in self.background_processes:
                ...
                if poll_result is None:
                    ...
                    with open(entry["stdout_path"]) as f:
                        stdout = f.read()
                    with open(entry["stderr_path"]) as f:
                        stderr = f.read()
        ...
        for pid, entry in list(self.background_processes.items()):
            ...
            if poll_result is None:
                ...
                try:
                    ...
                    processes.append({"pid": pid, "status": "running", "error": "Could not get process info"})
            else:
