...
class PythonTools:
    ...
    def execute_script(
        self, script_path: str, args: list[str] | None = None, cwd: str | None = None, timeout: float = 300.0
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            return {"success": False, "script": script_path, "error": str(e), "type": type(e).__name__}

    def execute_code(self, code: str, cwd: str | None = None, timeout: float = 300.0) -> dict[str, Any]:
        """
        ...
        try:
            ...
            result = self.execute_script(script_path, cwd=cwd, timeout=timeout)
    ...
    def install_packages(self, packages: list[str], upgrade: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": "Installation timed out", "packages": packages}
        except Exception as e:
            logger.error(f"Failed to install packages: {e}")
            return {"success": False, "error": str(e), "type": type(e).__name__, "packages": packages}
    ...
    def get_installed_packages(self) -> dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                return {"success": True, "packages": result.stdout, "venv_path": self.venv_path}
            else:
                return {"success": False, "error": "Failed to list packages", "stderr": result.stderr}
