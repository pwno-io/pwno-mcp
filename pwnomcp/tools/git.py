...
class GitTools:
    ...
    def fetch_repo(
        self, repo_url: str, version: str | None = None, target_dir: str | None = None, shallow: bool = True
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if version:
                ...
                fetch_result = subprocess.run(
                    ["git", "fetch", "origin", version], cwd=repo_path, capture_output=True, text=True
                )

                # Try to checkout the version
                checkout_result = subprocess.run(
                    ["git", "checkout", version], cwd=repo_path, capture_output=True, text=True
                )
            ...
            commit_result = subprocess.run(["git", "rev-parse", "HEAD"], cwd=repo_path, capture_output=True, text=True)
            current_commit = commit_result.stdout.strip() if commit_result.returncode == 0 else "unknown"

            # Get branch info
            branch_result = subprocess.run(
                ["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=repo_path, capture_output=True, text=True
            )
    ...
    def cleanup_workspace(self) -> dict[str, Any]:
        ...
        try:
            if os.path.exists(self.workspace_dir):
                ...
                os.makedirs(self.workspace_dir, exist_ok=True)
                return {"success": True, "message": "Workspace cleaned", "path": self.workspace_dir}
            else:
                return {"success": True, "message": "Workspace already clean", "path": self.workspace_dir}
        except Exception as e:
