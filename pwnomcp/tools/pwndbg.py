...
class PwndbgTools:
    ...
    def run(self, args: str = "", interrupt_after: float | None = None) -> dict[str, Any]:
        """
        ...
        if not self.session.binary_loaded:
            return {"success": False, "error": "No binary loaded. Use set_file first."}
    ...
    def step_control(self, command: str) -> dict[str, Any]:
        ...
        # Map command aliases
        command_map = {"c": "continue", "n": "next", "s": "step", "ni": "nexti", "si": "stepi"}
    ...
    def get_context(self, context_type: str = "all") -> dict[str, Any]:
        ...
        if self.gdb.get_state() != "stopped":
            return {"success": False, "error": f"Cannot get context while inferior is {self.gdb.get_state()}"}

        if context_type == "all":
            return {"success": True, "context": self._get_full_context()}
        else:
    ...
    def list_breakpoints(self) -> dict[str, Any]:
        ...
        return {"success": not result.get("error"), "breakpoints": breakpoints, "error": result.get("error")}
    ...
    def delete_breakpoint(self, number: int) -> dict[str, Any]:
        ...
        return {"success": not result.get("error"), "output": result["output"], "error": result.get("error")}
    ...
    def get_session_info(self) -> dict[str, Any]:
        ...
        if bp_result.get("breakpoints"):
            ...
            for bp in bp_result["breakpoints"]:
                self.session.add_breakpoint(int(bp.get("number", 0)), bp.get("addr", ""), bp.get("cond", ""))
