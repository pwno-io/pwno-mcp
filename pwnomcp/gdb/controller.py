...
class GdbController:
    ...
    def __init__(self, pwnodbg: str = "pwndbg"):
        ...
        """
        self.controller = gdbcontroller.GdbController(command=[pwnodbg, "--interpreter=mi3", "--quiet"])
    ...
    def execute_command(self, command: str, timeout_sec: float = 10.0) -> dict[str, Any]:
        ...
        while True:
            responses = self.controller.get_gdb_response(timeout_sec=timeout_sec, raise_error_on_timeout=False)
    ...
    def get_context(self, context_type: str) -> dict[str, Any]:
        ...
        if self._state != "stopped":
            return {"error": f"Cannot get context while inferior is {self._state}", "context_type": context_type}

        response = self.execute_command(f"context {context_type}")
        return {"context_type": context_type, "data": response["output"], "error": response["error"]}
    ...
    def interrupt(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "output": "".join(output_lines), "state": self._state}
